...
//                     cout << endl;
                    ...
                    double dist_inc = 0.5;	//每20ms经过两个点，依据速度<=50,所以插值距离是0.5m
                    ...
                    //s 路径的长度，x,y是map 坐标系下的
                    //插入的依据的s 路径的长度；一次只可以插入一个维度，分别是 x,y dx ,dy
                    ...
                    //PREVIOUS_PATH_POINTS_TO_KEEP=25
                    ...
                    else
                    {
                        ...
                        //得到当前点和最近的插入的waypoints的frenet coordinae values
                        //两个计算方式不一样
                        ...
                        //vector<double> frenet1 = getFrenet(pos_x, pos_y, angle, map_waypoints_x, map_waypoints_y, map_waypoints_s);
						pos_s = frenet[0];
						pos_d = frenet[1];
                        ...
                        //可能暂时用到,是一种替代方案
                        //my_car.s_traj_coeffs 是在generate_traj_for_target里面更新的
                    ...
                    //sendor fusion [id, x,y,vx,vy,s,d]
					...
					// cout << endl; 
                    // ********************* GENERATE trajectory candidates and choose the best one  **************************
                    // ********************* get best target s and d ,state,和路径**************************
					...
					for (string state: my_car.available_states) {
                        //有待进一步深究，这里才用到prediction的东西和车的当前状态
                        //内部有点问题
